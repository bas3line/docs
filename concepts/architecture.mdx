...
└──────────────────┬──────────────────────────────────────┘
        ...
        ▼                     ▼            ▼
┌───────────────┐    ┌────────────────┐   ┌───────────────┐
│   Selector    │    │  Server Pool   │   │   Metrics     │
│               │    │                │   │               │
│ • Round Robin │    │ • Backend list │   │ • Counters    │
│ • Least Conn  │    │ • Health state │   │ • Timers      │
│ • IP Hash     │    │ • Connections  │   │ • Histograms  │
│ • Weighted    │    │                │   │               │
│ • Random      │    │                │   │               │
└───────────────┘    └────────────────┘   └───────────────┘
        │                     │                    │
...
Internet → CDN
           └─→ UltraBalancer (L7)
                └─→ [App Servers]
                     └─→ UltraBalancer (DB Proxy)
                          └─→ [Database Replicas]
```

Layered load balancing for complex applications.
