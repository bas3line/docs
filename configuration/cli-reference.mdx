...
Supported Algorithms:
  ...
  • power-of-two     - Two random choices, pick least loaded
  • fastest-response - Fastest recent response time
...
Features:
  ...
  • Grafana/Prometheus dashboard (see 'ultrabalancer dashboard --help')
```

### dashboard

Deploy a complete monitoring stack with Grafana and Prometheus.

**Syntax:**
```bash
ultrabalancer dashboard [OPTIONS]
```

**Options:**
- `--start` - Start the monitoring dashboard (interactive setup)
- `--stop` - Stop the monitoring dashboard
- `--restart` - Restart the monitoring dashboard
- `--reset` - Stop dashboard and remove all data
- `--status` - Show dashboard status
- `--logs` - Show dashboard logs (Ctrl+C to exit)
- `--edit` - Edit dashboard configuration
- `--config <FILE>` - Load configuration from file
- `--ultrabalancer-host <HOST>` - UltraBalancer metrics host
- `--ultrabalancer-port <PORT>` - UltraBalancer metrics port
...
```bash Quick Start with Defaults
ultrabalancer dashboard --start \
...
**What it does:**
1. Creates `dashboard/` directory with all necessary files
2. Generates `docker-compose.yml` for Prometheus and Grafana
3. Creates pre-configured Prometheus scrape configuration
4. Provisions Grafana dashboard with UltraBalancer metrics
5. Auto-configures Prometheus as Grafana datasource
6. Starts Docker containers for monitoring stack
